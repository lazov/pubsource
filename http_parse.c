...
static int http_parse_uri_path(struct http_request *restrict request, size_t path_pos)
{
	...
	if (query_start)
	{
		...
		if (!json_raw.data) return InternalServerError;
...
// Given a URI, this function sets protocol, port, path, query (and optionally - the host header).
// The passed argument should be initialized with 0.
...
int http_parse_uri(struct http_request *restrict request)
{
	...
	if (!length) // Empty URI
	{
		if (!string_init(&request->path, "/", 1)) return InternalServerError;
		request->query = 0;
		return 0;
	}
	...
	else
	{
		...
		// Parse URI scheme.
		size_t index;
		const str http = string("http"), separator = string("://");
		...
		str name = string("host"), *value = string_alloc(host, host_length);
...
static int http_parse_header_field(struct dict *restrict headers, const char *buffer, unsigned length)
{
	...
	for(i = 0; i < length; ++i)
	{
		if (buffer[i] == ':') // If this is the end of field name
		{
			...
			if (!name.data) return InternalServerError;
			...
			else return InternalServerError; // memory error
		}
	}
...
int http_parse_header(struct dict *restrict header, struct stream *restrict stream)
{
	...
	if (!dict_init(header, DICT_SIZE_BASE)) return InternalServerError;
...
int http_parse_options(struct dict *restrict options, const str *string)
{
	size_t index = 0;
...
next:
		...
		value = malloc(sizeof(str) + sizeof(char) * (value_length + 1)); // TODO: this can be optimized to allocate less memory
		if (!value) return InternalServerError;
...
int http_parse_content_disposition(struct dict *restrict options, const str *string)
{
	size_t index = 0;
	size_t name_index, value_index, value_length;
	str name, *value;
	...
	while (1)
	{
		...
		// Allocate memory for NUL terminated value and copy the url-decoded value there
		value_length = index - value_index;
		value = malloc(sizeof(str) + sizeof(char) * (value_length + 1));
		if (!value) return InternalServerError;
		value->data = (char *)(value + 1);
		if (value_length)
		{
...
bool parse_context(struct context *restrict context)
{
...
{
	str key = string(buffer, separator);
	buffer += separator + 1;
	length -= separator + 1;
	str *value = malloc(sizeof(str) + length + 1);
...
int parse(struct context *restrict context)
{
	str buffer;
	size_t cached = stream_cached(&context->stream);
	if (cached >= 4096) return RequestEntityTooLarge; // TODO: change this
	if (!stream_read(&context->stream, &buffer, cached + 1)) return RequestTimeout; // TODO
	...
	for(; context->index < buffer.length; context->index += 1)
	{
		...
		case S_H: // space after URI
			...
			*format_bytes(context->request.URI.data, token.data, token.length) = 0;
			...
			stream_read_flush(&context->stream, context->index);
			stream_read(&context->stream, &buffer, buffer.length - context->index);
			context->index = 0;
		...
		case FIN:
			stream_read_flush(&context->stream, context->index + 1);
			return 0; // success
...
int http_parse(struct http_request *restrict request, struct stream *restrict stream)
{
...
static int http_parse_accept_add(const char *buffer, size_t item_length, size_t quality_length, str *restrict list, unsigned *priorities, size_t *restrict allow, size_t *restrict deny)
{
	int priority;
	str item;
	size_t last;
...
int http_parse_accept(const str *header, str **list, size_t *restrict allow, size_t *restrict deny)
{
	size_t index;
	...
	*deny = 0;
	...
	free(priorities);
	count = *allow + *deny;
	*list = realloc(*list, sizeof(str) * count);
